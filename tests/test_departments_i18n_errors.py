...
@pytest.mark.asyncio
async def test_bulk_slot_invalid_time_range_error_is_i18n_ready(client):
    create = await client.post(
        "/api/departments",
        json={"name": "Library", "code": "LIB"},
    )
    assert create.status_code == 201
    department_id = create.json()["department"]["id"]
    ...
    assert resp.status_code == 422
    detail = resp.json()["detail"]
    assert detail["message_key"] == "departments.time_slot.invalid_time_range"
    assert detail["params"]["start_time"] == "17:00:00"
    assert detail["params"]["end_time"] == "09:00:00"
