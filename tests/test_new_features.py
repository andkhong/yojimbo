...
@pytest.mark.asyncio
async def test_options_with_origin_but_no_preflight_header_does_not_get_cors_headers(client):
    """Only real preflight OPTIONS requests should receive CORS allow headers."""
    resp = await client.options(
        "/api/departments",
...
async def test_cors_allowlist_enforced_in_production(client, monkeypatch):
    ...
    allowed = await client.options(
        "/api/departments",
        headers={"origin": "https://dashboard.city.gov", "access-control-request-method": "GET"},
    )
