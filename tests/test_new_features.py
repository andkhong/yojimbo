...
@pytest.mark.asyncio
async def test_cors_headers_not_applied_to_non_api_paths(client, monkeypatch):
    """CORS headers should only be emitted for API routes."""
    monkeypatch.setenv("DEBUG", "false")
    monkeypatch.setenv("CORS_ALLOWED_ORIGINS", "https://dashboard.city.gov")

    resp = await client.get("/dashboard", headers={"origin": "https://dashboard.city.gov"})
    assert resp.status_code in (200, 307)
    assert "access-control-allow-origin" not in resp.headers


@pytest.mark.asyncio
async def test_options_non_api_path_does_not_use_cors_preflight_shortcut(client, monkeypatch):
    """Non-API OPTIONS should not return middleware-generated 204 preflight responses."""
    monkeypatch.setenv("DEBUG", "false")
    monkeypatch.setenv("CORS_ALLOWED_ORIGINS", "https://dashboard.city.gov")

    resp = await client.options("/dashboard", headers={"origin": "https://dashboard.city.gov"})
