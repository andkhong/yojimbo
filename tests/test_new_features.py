...
@pytest.mark.asyncio
async def test_cors_not_applied_to_non_api_routes(client, monkeypatch):
    """CORS middleware should not emit API CORS headers on non-/api routes."""
    monkeypatch.setenv("DEBUG", "false")
    monkeypatch.setenv("CORS_ALLOWED_ORIGINS", "https://dashboard.city.gov")

    resp = await client.options("/", headers={"origin": "https://dashboard.city.gov"})
