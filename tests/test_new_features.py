...
@pytest.mark.asyncio
async def test_cors_headers_not_added_on_non_api_paths(client, monkeypatch):
    """Non-API endpoints should not emit CORS response headers."""
    monkeypatch.setenv("DEBUG", "false")
    monkeypatch.setenv("CORS_ALLOWED_ORIGINS", "https://dashboard.city.gov")
...
@pytest.mark.asyncio
async def test_options_non_api_path_not_short_circuited_by_cors_preflight(client):
    """OPTIONS on non-API paths should pass through routing (not forced to 204)."""
    resp = await client.options(
        "/login",
        headers={"origin": "https://dashboard.city.gov"},
    )
